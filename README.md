...
**! TODO:** Хардкодить настройки, наверное, плохая идея. Как привязаться к экрану, чтобы приложение не превышало максимально допустимый размер?

2) при работе с координатами нужно ли эксплицитно приводить их к float-типу? Например:
    ````
    batch.draw(img, SCREEN_WIDTH / 2 - (float) img.getWidth() / 2, SCREEN_HEIGHT / 2 - (float) img.getHeight() / 2);

3) настройка setColor и позиционирование:
...
### Урок 2. Работа с графикой. Векторная математика. Обработка логики игры.
